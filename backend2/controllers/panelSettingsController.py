...
@layout_ns.route("/updatePanelSetting")
class UpdatePanelSetting(Resource):
    """Updates a panel setting."""

    def post(self):
        data = request.json
        setting = data.get("setting")
        value = data.get("value")
        if setting not in PanelSettingType:
            return jsonify({"success": False, "error": "Invalid setting."}), 400

        setting_data = PanelSettings.query.filter_by(setting=setting).first()
        if setting_data:
            setting_data.value = value
        else:
            setting_data = PanelSettings(setting=setting, value=value)
            db.session.add(setting_data)

        db.session.commit()
        return jsonify({"success": True, "message": "Setting updated successfully."}), 200

@layout_ns.route("/getPanelSetting")
class GetPanelSetting(Resource):
    """Retrieves a panel setting."""
    ...
    def get(self):
        ...
        return jsonify({"success": True, "value": setting_data.value}), 200
